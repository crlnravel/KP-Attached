...
| `ATTACHED_ADMIN_PASSWORD` | Override the default local admin password. |
| `ATTACHED_MODEL_ROOT` | Absolute path to `data_model_KP` when it is not in the default sibling location. |
| `ATTACHED_PROJECT_ROOT` | Override project-root detection used to locate `data_model_KP`. |
